...
class Trainer:
    ...
    def start_game(self):
        ...
        # f-string for logging selected map
        logger.info(f"Training on map {map_id} ...")
        self.game.start(
            map_id=map_id,
    ...
    def run(self):
        ...
        while True:
            ...
            if (
                dump_frequency > 0
                and (self.n_iter - last_dump_iter) % dump_frequency == 0
            ):
            ...
            for p in self.network.module.parameters():
                if p.grad is not None:
                    p.grad.clamp_(-5, 5)  # modernization: operate on grad directly
    ...
    def evaluate_model(self, start_iter):
        ...
        new_score = self.eval_fn(self.game, self.network, self.params, self.n_iter)
        if new_score > self.best_score:
            self.best_score = new_score
            logger.info(f"New best score: {self.best_score:f}")
            model_name = f"best-{self.n_iter - start_iter}.pth"
            model_path = os.path.join(self.params.dump_path, model_name)
            logger.info(f"Best model dump: {model_path}")
            torch.save(self.network.module.state_dict(), model_path)
        self.network.module.train()
        self.start_game()

    def dump_model(self, start_iter):
        model_name = f"periodic-{self.n_iter - start_iter}.pth"
        model_path = os.path.join(self.params.dump_path, model_name)
        logger.info(f"Periodic dump: {model_path}")
        torch.save(self.network.module.state_dict(), model_path)
    ...
    def sync_update_parameters(self):
        ...
        grad_scale = 1.0 / server.n_processes
        if server.rank == 0:
            # accumulate shared gradients into the local copy
            for k in self.state_dict:
                self.state_dict[k].grad.mul_(grad_scale).add_(
                    shared_dict[k].grad
                )  # modernization: keep API-compatible arithmetic
            # do optimization
            self.optimizer.step()
            ...
            for k in shared_dict:
                shared_dict[k].grad.add_(
                    self.state_dict[k].grad, alpha=grad_scale
                )  # modernization: use alpha kwarg
...
class ReplayMemoryTrainer(Trainer):
    ...
    def training_step(self, current_loss):
        ...
        memory = self.replay_memory.get_batch(
            ...
            self.params.hist_size
            + (0 if self.params.recurrence == "" else self.params.n_rec_updates - 1),
        )
