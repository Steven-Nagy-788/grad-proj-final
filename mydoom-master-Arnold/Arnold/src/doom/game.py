...
class Game:
    def __init__(
        ...
        """
        # game resources
        # Prefer f-strings over legacy percent formatting
        game_filename = f"{'freedoom2' if freedoom else 'Doom2'}.wad"
        self.scenario_path = os.path.join(RESOURCES_DIR, "scenarios", f"{scenario}.wad")
        ...
        assert n_bots >= 0
        assert (
            type(use_scripted_marines) is bool
            or use_scripted_marines is None
            and n_bots == 0
        )
    ...
    def update_game_variables(self):
        ...
        assert all(
            v.is_integer() or k[-2:] in ["_x", "_y", "_z"] for k, v in new_v.items()
        )
    ...
    def update_statistics_and_reward(self, action):
        ...
        if d != 0:
            ...
            # f-string for clearer event log
            self.log(
                f"{'Found' if d > 0 else 'Lost'} health "
                f"({self.prev_properties['health']} -> {self.properties['health']})"
            )
        ...
        if d != 0:
            if d > 0:
                ...
                stats["armors"] += 1
            self.log(
                f"{'Found' if d > 0 else 'Lost'} armor ({self.prev_properties['armor']} -> {self.properties['armor']})"
            )

        # change weapon
        if self.properties["sel_weapon"] != self.prev_properties["sel_weapon"]:
            self.log(
                "Switched weapon: {} -> {}".format(
        ...
        for i, weapon in enumerate(
            ...
            stats[weapon] += 1
            self.log(f"Found weapon: {WEAPON_NAMES[i + 1]}")
        ...
        for ammo in ["bullets", "shells", "rockets", "cells"]:
            ...
            if d != 0:
                ...
                self.log(
                    f"{'Found' if d > 0 else 'Lost'} ammo: {ammo} "
                    f"({self.prev_properties[ammo]} -> {self.properties[ammo]})"
                )
    ...
    def start(self, map_id, episode_time=None, manual_control=False, log_events=False):
        ...
        assert map_id > 0
        self.map_id = map_id
        # Use explicit zero-padded formatting with f-strings
        ...
        if self.players_per_game > 1:
            ...
            if self.player_rank % self.players_per_game == 0:
                # f-string for building host/port arguments
                args.append(f"-host {self.players_per_game} -port {port}")
            else:
                # f-string for join argument
                args.append(f"-join 127.0.0.1:{port}")
        else:
            args.append("-host 1")
        ...
        self.game.set_labels_buffer_enabled(
            self.use_labels_buffer or self.use_game_features
        )
        ...
        args.append(f"+sv_respawnprotect {self.respawn_protect}")
        args.append(f"+sv_spawnfarthest {self.spawn_farthest}")

        # freelook / agent name / agent color
        # Keep numeric conversion explicit for clarity
        args.append(f"+freelook {1 if self.freelook else 0}")
        args.append(f"+name {self.name}")
        args.append(f"+colorset {self.color}")
    ...
    def reset(self):
        ...
        while self.is_player_dead():
            # f-string for warning to avoid percent formatting
            logger.warning(
                f"Player {self.params.player_rank} is still dead after respawn."
            )
            self.respawn_player()
    ...
    def update_bots(self):
        ...
        if self.use_scripted_marines:
            # Build ACS command with f-string
    ...
    def randomize_textures(self, randomize):
        ...
        # Use f-string for ACS command
        self.game.send_game_command(f"pukename set_value always 4 {randomize}")
    ...
    def init_bots_health(self, health):
        ...
        assert 0 < health <= 100
        # Use f-string for ACS command
        self.game.send_game_command(f"pukename set_value always 5 {health}")
    ...
    def make_action(self, action, frame_skip=1, sleep=None):
        ...
        for weapon_name, weapon_ammo, weapon_id in WEAPONS_PREFERENCES:
            ...
            if (
                self.properties[weapon_name] > 0
                and self.properties[weapon_ammo] >= min_ammo
            ):
                if self.properties["sel_weapon"] != weapon_id:
                    # Build key using f-string instead of legacy formatting
                    switch_action = (
                        [False] * self.mapping[f"SELECT_WEAPON{weapon_id}"]
                    ) + [True]
                    action = action + switch_action[len(action) :]
                    self.log(
                        f"Manual weapon change: {WEAPON_NAMES[self.properties['sel_weapon']]} -> {weapon_name}"
                    )
                break
        ...
        if self.manual_control and (
            self.count_non_forward_actions >= 30 or self.count_non_turn_actions >= 60
        ):
    ...
    def print_statistics(self, eval_time=None):
        ...
        self.statistics["all"] = {
            k: sum(v[k] for v in self.statistics.values()) for k in self.stat_keys
        }
        ...
        # statistics to log
        log_lines = [
            # Use f-strings to build map header names
            [""] + [f"Map{i:02d}" for i in map_ids] + ["All"],
            ("Kills", "kills"),
            ("Deaths", "deaths"),
            ("Suicides", "suicides"),
            ("Frags", "frags"),
        ...
        for line in log_lines:
            if line is None:
                ...
                line = [f"{x:.3f}" if type(x) is float else x for x in line]
