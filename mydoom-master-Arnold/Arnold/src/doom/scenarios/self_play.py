...
def main(_parser, args):
    ...
    parser.add_argument("--execute", default="deathmatch_rockets", help="Script to run")
    parser.add_argument(
        "--num_players", type=int, default=2, help="Number of agents to run"
    )
    parser.add_argument(
        "--num_games", type=int, default=1, help="Number of games to run"
    )
    params, remaining_args = parser.parse_known_args(args)
    module = importlib.import_module("...scenarios." + params.execute, package=__name__)
    ...
    for i in range(params.num_players):
        subprocess_args = [
            "--players_per_game",
            str(players_per_game),
            "--player_rank",
            str(i),
        ]
        subprocess_args += remaining_args
        proc = mp.Process(
            target=worker_fn_factory(module.main),
