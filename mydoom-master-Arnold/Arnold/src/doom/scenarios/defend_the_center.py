...
def main(parser, args, parameter_server=None):
    ...
    # Action builder
    assert params.action_combinations in [
        "turn_lr+attack",
        "attack+turn_lr",
        "turn_lr;attack",
        "attack;turn_lr",
...
def evaluate_defend_the_center(game, network, params, n_train_iter=None):
    ...
    while True:
        ...
        if game.is_player_dead() or game.is_episode_finished():
            ...
            logger.info(f"{game.properties['score']} kills.")
            logger.info("===============")
    ...
    logger.info(f"{n_iter} iterations on {len(n_kills)} episodes.")
    if n_features != 0:
        gf_confusion.print_statistics()
    logger.info(f"Kills by episode: {str(n_kills)}")
    logger.info(f"{np.mean(n_kills):f} kills / episode average.")
    to_log = {
        "min_n_kills": float(np.min(n_kills)),
