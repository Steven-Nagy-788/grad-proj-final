...
def register_scenario_args(parser):
    ...
    """
    parser.add_argument("--wad", type=str, default="", help="WAD scenario filename")
    parser.add_argument(
        "--n_bots", type=int, default=8, help="Number of ACS bots in the game"
    )
    parser.add_argument("--reward_values", type=str, default="", help="reward_values")
    parser.add_argument(
        "--randomize_textures",
        type=bool_flag,
        default=False,
    ...
    parser.add_argument(
        "--eval_model_path", type=str, default="", help="Models locations"
    )
    parser.add_argument("--eval_model_ids", type=str, default="", help="Models name")
    parser.add_argument(
        "--eval_time", type=int, default=100, help="Evaluation time (in seconds)"
    )
...
def main(parser, args, parameter_server=None):
    ...
    logger.info("\n".join(f"{k}: {str(v)}" for k, v in dict(vars(params)).items()))
...
def evaluate_deathmatch(game, network, params, n_train_iter=None):
    ...
    for map_id in params.map_ids_test:
        logger.info(f"Evaluating on map {map_id} ...")
        ...
        while n_iter * params.frame_skip < params.eval_time * 35:
            ...
            while game.is_player_dead():
                # f-string for warning message
                logger.warning(
                    f"Player {params.player_rank} is still dead after respawn."
                )
                game.respawn_player()
            ...
            if n_features:
                ...
                pred_features = (
                    pred_features.detach().cpu().numpy().ravel()
                )  # modernization: use detach() instead of .data
                confusion.update_predictions(
                    pred_features, last_states[-1].features, game.map_id
                )
        ...
        # close the game
        game.close()
        logger.info(f"{n_iter} iterations")
    ...
    logger.info(f"__log__:{json.dumps(to_log)}")
