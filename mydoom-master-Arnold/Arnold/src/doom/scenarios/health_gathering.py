...
def main(parser, args, parameter_server=None):
    ...
    # Initialize the game
    game = Game(
        scenario=f"health_gathering{'_supreme' if params.supreme else ''}",
...
def evaluate_health_gathering(game, network, params, n_train_iter=None):
    ...
    while True:
        ...
        if game.is_player_dead() or game.is_episode_finished():
            ...
            # f-string for logging numeric value
            logger.info(f"Survived for {game.game.get_episode_time()} steps.")
            logger.info("===============")
        ...
        if n_features:
            ...
            gf_confusion.update_predictions(
                pred_features, last_states[-1].features, map_id
            )
    ...
    logger.info(f"{n_iter} iterations on {len(survival_time)} episodes.")
    if n_features != 0:
        gf_confusion.print_statistics()
    logger.info(f"Survival time by episode: {str(survival_time)}")
    logger.info(f"{np.mean(survival_time):f} survival time / episode average.")
    to_log = {
        "min_survival_time": float(np.min(survival_time)),
