...
def process_buffers(game, params):
        ...
        if gray:
            ...
            if screen_buffer.shape != (height, width):
                screen_buffer = cv2.resize(
                    screen_buffer, (width, height), interpolation=cv2.INTER_AREA
                )
            screen_buffer = screen_buffer.reshape(1, height, width).astype(np.uint8)
        else:
            # resize
    ...
    if game.use_depth_buffer:
        ...
        if depth_buffer.shape != (height, width):
            depth_buffer = cv2.resize(
                depth_buffer, (width, height), interpolation=cv2.INTER_AREA
            )
    ...
    if game.use_labels_buffer or game.use_game_features:
        ...
        if game.use_game_features:
            ...
            visible = int(visible)
            game_features = [
                visible & (1 << i) > 0 for i, x in enumerate(game.game_features) if x
            ]
            if params.dump_freq == 30003:
                label_game_features = [
                    np.any(x)
                    for i, x in enumerate(__labels_buffer)
                    if game.game_features[i + 1]
                ]
        ...
        if game.use_labels_buffer:
            n_feature_maps = max(x for x in game.labels_mapping if x is not None) + 1
            if n_feature_maps == 4:
                _labels_buffer = __labels_buffer
            else:
                _labels_buffer = np.zeros(
                    (n_feature_maps,) + init_shape, dtype=np.uint8
                )
                for i in range(4):
