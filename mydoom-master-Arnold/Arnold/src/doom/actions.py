...
class ActionBuilder:
    def __init__(self, params):
        ...
        # Modernize string formatting to f-strings for clarity and performance
        logger.info(
            f"{len(self.available_buttons)} available buttons: {self.available_buttons}"
        )

        if params.use_continuous:
            assert params.speed == "manual"
            self.available_actions = create_action_set(
                params.action_combinations, params.use_continuous
            )
            if params.crouch == "manual":
                self.available_actions.append("CROUCH")
            # number of continuous / discrete actions
            params.n_continuous = sum(["_DELTA" in x for x in self.available_actions])
            params.n_discrete = len(self.available_actions) - params.n_continuous
        else:
        ...
        # Use f-strings instead of percent formatting for Python 3.12 best practices
        logger.info(
            f"{self.n_actions} available actions:\n"
...
def create_action_set(action_combinations, use_continuous):
    ...
    if use_continuous:
        ...
        action_set = [
            action_categories_continuous[x] for x in action_combinations.split(";")
        ]
        ...
        for subset2 in action_subsets:
            action_set = sum(
                [[subset1 + [x] for x in subset2] for subset1 in action_set], []
            )
...
def add_buttons(game, buttons):
    ...
    for i in range(10):
        # Prefer f-strings over interpolation with % to avoid deprecation and improve readability
        buttons.append(f"SELECT_WEAPON{i}")
