...
class DQNRecurrent(DQN):
    ...
    def __init__(self, params):
        super().__init__(params)
        h_0 = torch.FloatTensor(
            params.n_rec_layers, params.batch_size, params.hidden_dim
        ).zero_()
        self.init_state_t = self.get_tensor(h_0)
        with torch.no_grad():  # modernization: volatile is removed; use no_grad scope
            init_slice = self.init_state_t[:, :1, :].clone()
        self.init_state_e = init_slice
    ...
    def f_eval(self, last_states):
        ...
        if self.params.remember:
            output = self.module(
                ...
                [variables[-1:, i].view(1, 1) for i in range(self.params.n_variables)],
    ...
    def f_train(
        self, screens, variables, features, actions, rewards, isfinal, loss_history=None
    ):
        screens, variables, features, actions, rewards, isfinal = (
            self.prepare_f_train_args(
                screens, variables, features, actions, rewards, isfinal
            )
        )
        ...
        scores1 = output_sc.masked_select(self.get_tensor(mask))
        scores2 = rewards + (
        ...
        loss_sc = self.loss_fn_sc(
            ...
            scores2.detach()[
                :, -self.params.n_rec_updates :
            ],  # modernization: detach target instead of Variable
        )
    ...
    def register_args(parser):
        ...
        parser.add_argument(
            "--n_rec_updates", type=int, default=1, help="Number of updates to perform"
        )
        parser.add_argument(
            "--n_rec_layers", type=int, default=1, help="Number of recurrent layers"
        )
        parser.add_argument(
            "--remember",
            type=bool_flag,
