...
class DQNModuleBase(nn.Module):
    def __init__(self, params):
        ...
        # Use f-strings for logging numeric values
        logger.info(f"Conv layer output dim : {self.conv_output_dim}")
        logger.info(f"Hidden layer input dim: {self.output_dim}")
...
class DQN:
    ...
    def get_tensor(self, x):
        """Modernization: Variables are deprecated; tensors track grad directly."""
    ...
    def log_loss(self, loss_history):
        logger.info("DQN loss: {:.5f}".format(np.mean(loss_history["dqn_loss"])))
        if self.n_features > 0:
            logger.info(
                "Game features loss: {:.5f}".format(np.mean(loss_history["gf_loss"]))
            )
    ...
    def prepare_f_train_args(
        self, screens, variables, features, actions, rewards, isfinal
    ):
        """
        Prepare inputs for training.
        """
        # convert tensors to torch tensors (Variables removed in modern PyTorch)
        screens = self.get_tensor(
            torch.FloatTensor(np.float32(screens).copy())
        )  # modernization: use tensors directly
        if self.n_variables:
            variables = self.get_tensor(torch.LongTensor(np.int64(variables).copy()))
        if self.n_features:
            features = self.get_tensor(torch.LongTensor(np.int64(features).copy()))
        rewards = self.get_tensor(torch.FloatTensor(np.float32(rewards).copy()))
        isfinal = self.get_tensor(torch.FloatTensor(np.float32(isfinal).copy()))
    ...
    def register_loss(self, loss_history, loss_sc, loss_gf):
        loss_history["dqn_loss"].append(
            loss_sc.item()
        )  # modernization: .item() replaces .data[0]
        loss_history["gf_loss"].append(
            loss_gf.item() if self.n_features else 0
        )  # modernization: .item()
    ...
    def next_action(self, last_states, save_graph=False):
        ...
        action_id = int(
            torch.argmax(scores, dim=0).item()
        )  # modernization: replace .data.max(...) with argmax().item()
    ...
    def register_args(parser):
        ...
        parser.add_argument(
            "--replay_memory_size", type=int, default=1000000, help="Replay memory size"
        )
        ...
        parser.add_argument(
            "--final_decay", type=float, default=0.1, help="Epsilon value after decay"
        )
