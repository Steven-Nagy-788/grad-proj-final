...
def get_recurrent_module(module_type):
    if module_type == "rnn":
        ...
        raise Exception(f"Unknown recurrent module type: '{module_type}'")
...
def value_loss(delta):
    ...
    if delta == 0:
        ...
        def loss_fn(input, target):
            ...
            diff_delta = torch.clamp(
                diff, max=delta
            )  # modernization: replace deprecated cmin with clamp
...
def build_CNN_network(module, params):
    ...
    width = params.width
    logger.info(f"Input shape: {str((params.n_fm, height, width))}")
    ...
    )  # modernization: Variables deprecated; use tensors
...
def build_game_variables_network(module, params):
    ...
    for i, (name, n_values) in enumerate(params.game_variables):
        embeddings = BucketedEmbedding(
            params.bucket_size[i], n_values, params.variable_dim[i]
        )
        setattr(module, f"{name}_emb", embeddings)
