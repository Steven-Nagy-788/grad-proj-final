...
models = {"dqn_ff": DQNFeedforward, "dqn_rnn": DQNRecurrent}
...
def get_model_class(model_type):
    ...
    if cls is None:
        ...
        raise RuntimeError(
            f"unknown model type: '{model_type}'. supported values are: {', '.join(models.keys())}"
        )
    return cls
...
def register_model_args(parser, args):
    ...
    parser.add_argument("--clip_delta", type=float, default=1.0, help="Clip delta")
    parser.add_argument(
        "--variable_dim",
        type=str,
        default="32",
    ...
    parser.add_argument(
        "--bucket_size", type=str, default="1", help="Bucket size for game variables"
    )
    parser.add_argument(
